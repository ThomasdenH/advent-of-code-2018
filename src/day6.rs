...
impl AreaOfIndexMap {
    ...
    fn increase_by_one(&mut self, index: usize) {
        match self.0.get(&index) {
            None => { self.0.insert(index, Area::Finite(1)); },
            Some(Area::Finite(current_area)) => { self.0.insert(index, Area::Finite(current_area + 1)); },
            _ => {}
        }
    }
...
fn biggest_area(input: &Vec<(u32, u32)>) -> u32 {
    ...
    while let Some(QueueEntry{index, coord, distance}) = queue.pop_front() {
        if coord.0 < min_x
            || coord.0 > max_x
            || coord.1 < min_y
            || coord.1 > max_y {
            ...
            match map.get_mut(&coord) {
                None => { // There is not yet a closest plot.
                    // Draw on map...
                    map.insert(coord, AreaPlot {
                ...
                Some(area_plot) => { // There is another plot closer or as close

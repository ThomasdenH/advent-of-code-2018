...
lazy_static! {
    static ref REGEX_BEGIN_SHIFT: Regex = Regex::new(r"(?x)\[
        (?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})
        \ 
        (?P<hour>\d{2}):(?P<minute>\d{2})
        \]\ 
        Guard\ \#(?P<guard>\d+)\ begins\ shift").unwrap();
}

lazy_static! {
    static ref REGEX_FALLS_ASLEEP: Regex = Regex::new(r"(?x)\[
        (?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})
        \ 
        (?P<hour>\d{2}):(?P<minute>\d{2})
        \]\ 
...
lazy_static! {
    static ref REGEX_WAKES_UP: Regex = Regex::new(r"(?x)\[
        (?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})
        \ 
        (?P<hour>\d{2}):(?P<minute>\d{2})
        \]\ 
...
#[test]
fn test_log_parse() {
    assert_eq!("[1518-11-01 00:00] Guard #10 begins shift".parse::<LogEntry>().unwrap(), LogEntry {
        date: Date {
            year: 1518,
            month: 11,
            day: 1,
    ...
    assert_eq!("[1518-09-26 23:59] Guard #2851 begins shift".parse::<LogEntry>().unwrap(), LogEntry {
        date: Date {
            year: 1518,
            month: 9,
            day: 26,
    ...
    assert_eq!("[1518-11-01 00:05] falls asleep".parse::<LogEntry>().unwrap(), LogEntry {
        date: Date {
            year: 1518,
            month: 11,
            day: 1,
    ...
    assert_eq!("[1518-11-01 00:25] wakes up".parse::<LogEntry>().unwrap(), LogEntry {
        date: Date {
            year: 1518,
            month: 11,
            day: 1,
